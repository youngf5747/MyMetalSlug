...
import com.simplegame.mymetalslug.utils.RandomUtil;
...
public class Monster {
    ...
    public static final int TYPE_BOMB = 1; // 地雷
    public static final int TYPE_FLY = 2; // 飞机
    public static final int TYPE_MAN = 3; // 人形怪物
    // 定义怪物类型的成员变量
    ...
    public Monster(int type)
    {
        ...
        {
            y = (GameManager.SCREEN_HEIGHT >> 1)
                    - RandomUtil.rand((int) (GameManager.scale * 100));
        }
        // 随机计算怪物的X坐标。
        x = GameManager.SCREEN_WIDTH + RandomUtil.rand(GameManager.SCREEN_WIDTH >> 1)
    ...
    public void draw(Canvas canvas)
    {
        ...
        switch (type)
        {
            case TYPE_BOMB:
                // 死亡的怪物用死亡图片
                drawAni(canvas, isDead ? GameManager.bomb2Image : GameManager.bombImage);
                break;
            case TYPE_FLY:
                // 死亡的怪物用死亡图片
                drawAni(canvas, isDead ? GameManager.flyDieImage : GameManager.flyImage);
                break;
            case TYPE_MAN:
                // 死亡的怪物用死亡图片
                drawAni(canvas, isDead ? GameManager.manDieImage : GameManager.manImage);
                break;
            default:
                break;
        }
    ...
    public void drawAni(Canvas canvas, Bitmap[] bitmapArr)
    {
        ...
        // 如果怪物已经死，且没有播放过死亡动画（dieMaxDrawCount等于初始值表明未播放过死亡动画）
        if (isDead && dieMaxDrawCount == Integer.MAX_VALUE)
        {
        ...
        if (isDead)
        {
            if (type == TYPE_BOMB)
            {
                drawX = x - (int) (GameManager.scale * 50);
            }
        ...
        // 控制动画刷新速度的计数器, 即经过几次Count, 怪物刷新下一帧
        drawCount++;
        // 后面6、4用于控制人、飞机的刷新动画速度
    ...
    public void addBullet()
    {
        ...
        // 计算子弹的X、Y坐标
        int drawX = x;
        int drawY = y - (int) (GameManager.scale * 60);
        // 如果怪物是飞机，重新计算飞机发射的子弹的Y坐标
        if (type == TYPE_FLY)
        {
            drawY = y - (int) (GameManager.scale * 30);
        }
        // 创建子弹对象
    ...
    // 更新所有monster和子弹的位置：将所有monster和子弹的X坐标减少shift距离（左移）
    ...
    public void setDead(boolean isDie)
    {
        this.isDead = isDie;
    }
