...
import com.simplegame.mymetalslug.utils.Graphics;
import com.simplegame.mymetalslug.utils.MonsterManager;
...
public class Player {
    ...
    // 当leftShootTime不为0时, 表示角色当前正处于射击状态; 变为0时，用户才能发射下一枪
    ...
    public void initPosition() {
        x = GameManager.SCREEN_WIDTH * 15 / 100;
        y = GameManager.SCREEN_HEIGHT * 75 / 100;
        X_DEFAULT = x;
        Y_DEFAULT = y;
        Y_JUMP_MAX = GameManager.SCREEN_HEIGHT * 50 / 100;
    }

    // 获取该角色当前方向：action成员变量为奇数代表向右
    public int getDir() {
        return action % 2 == 1 ? DIR_RIGHT : DIR_LEFT;
    }
    ...
    public void draw(Canvas canvas) {
        ...
        switch (action) {
            case ACTION_STAND_RIGHT:
                drawAni(canvas, GameManager.legStandImage, GameManager.headStandImage, DIR_RIGHT);
                break;
            case ACTION_STAND_LEFT:
                drawAni(canvas, GameManager.legStandImage, GameManager.headStandImage, DIR_LEFT);
                break;
            case ACTION_RUN_RIGHT:
                drawAni(canvas, GameManager.legRunImage, GameManager.headRunImage, DIR_RIGHT);
                break;
            case ACTION_RUN_LEFT:
                drawAni(canvas, GameManager.legRunImage, GameManager.headRunImage, DIR_LEFT);
                break;
            case ACTION_JUMP_RIGHT:
                drawAni(canvas, GameManager.legJumpImage, GameManager.headJumpImage, DIR_RIGHT);
                break;
            case ACTION_JUMP_LEFT:
                drawAni(canvas, GameManager.legJumpImage, GameManager.headJumpImage, DIR_LEFT);
                break;
            default:
                break;
        }
    ...
    public void drawAni(Canvas canvas, Bitmap[] legArr, Bitmap[] headArrFrom, int dir) {
        ...
        // 射击状态停留次数每次减1
        if (leftShootTime > 0) {
            headArr = GameManager.headShootImage;
            leftShootTime--;
        }
        ...
        Graphics.drawMatrixImage(canvas, bitmap, 0, 0, bitmap.getWidth(),
                bitmap.getHeight(), trans, drawX, drawY, 0, Graphics.TIMES_SCALE);
        ...
        if (action == ACTION_STAND_LEFT) {
            drawX += (int) (6 * GameManager.scale);
        }
        drawY = drawY - bitmap2.getHeight() + (int) (10 * GameManager.scale);
    ...
    public void drawHead(Canvas canvas) {
        ...
        // 画头像
        Graphics.drawMatrixImage(canvas, GameManager.head, 0, 0,
                GameManager.head.getWidth(), GameManager.head.getHeight(),
                Graphics.TRANS_MIRROR, 0, 0, 0, Graphics.TIMES_SCALE);
        Paint p = new Paint();
        p.setTextSize(30);
        // 画名字
        Graphics.drawBorderString(canvas, 0xa33e11, 0xffde00, name,
                GameManager.head.getWidth(), (int) (GameManager.scale * 20), 3, p);
        // 画生命值
        Graphics.drawBorderString(canvas, 0x066a14, 0x91ff1d, "HP: " + hp,
                GameManager.head.getWidth(), (int) (GameManager.scale * 40), 3, p);
    }
    ...
    public void drawBullet(Canvas canvas) {
        ...
        for (int i = 0; i < bulletList.size(); i++)
        {
            ...
            if (bullet.getX() < 0 || bullet.getX() > GameManager.SCREEN_WIDTH)
            {
    ...
    public void addBullet() {
        ...
        int bulletX = getDir() == DIR_RIGHT ? X_DEFAULT + (int)
                (GameManager.scale * 50) : X_DEFAULT - (int)(GameManager.scale * 50);
        // 创建子弹对象
        Bullet bullet = new Bullet(Bullet.BULLET_TYPE_1, bulletX,
                y - (int) (GameManager.scale * 60), getDir());
        // 将子弹添加到用户发射的子弹集合中
        bulletList.add(bullet);
        // 发射子弹时，将leftShootTime设置为射击状态最大值
        leftShootTime = MAX_LEFT_SHOOT_TIME;
        // 播放射击音效
        GameManager.soundPool.play(GameManager.soundMap.get(1), 1, 1, 0, 0, 1);
    }
    ...
    public void move() {
        if (move == MOVE_RIGHT) {
            // 更新怪物的位置
            MonsterManager.updatePosition((int) (6 * GameManager.scale));
            // 更新角色位置
            setX(getX() + (int) (6 * GameManager.scale));
            if (!isJump()) {
                // 不跳的时候，需要设置动作
                setAction(Player.ACTION_RUN_RIGHT);
            }
        } else if (move == MOVE_LEFT) {
            if (getX() - (int) (6 * GameManager.scale) < Player.X_DEFAULT) {
                // 更新怪物的位置
    ...
    public void logic() {
        ...
        if (!isJumpMax) {
            ...
            // 更新Y坐标
            setY(getY() - (int) (8 * GameManager.scale));
            // 设置子弹在Y方向上具有向上的速度
            setBulletYSpeed(-(int) (2 * GameManager.scale));
            // 已经达到最高点
    ...
    // 给子弹设置垂直方向上的速度
    // 游戏的设计是：当角色跳动时，子弹会具有垂直方向上的速度
    public void setBulletYSpeed(int speed)
    {
        for (Bullet bullet : bulletList)
        {
            if (bullet == null || bullet.getYSpeed() != 0)
            {
                continue;
            }
    ...
    {
        this.x = x % (GameManager.map.getWidth() + X_DEFAULT);
        // 如果角色移动到屏幕最左边
        if (this.x < X_DEFAULT)
        {
            this.x = X_DEFAULT;
