...
import com.simplegame.mymetalslug.bean.Bullet;
import com.simplegame.mymetalslug.bean.Monster;
...
public class MonsterManager {
    ...
    {
        if (monsterList.size() < 3 + RandomUtil.rand(3))
        {
            // 创建新怪物
            Monster monster = new Monster(1 + RandomUtil.rand(3));
        ...
        for (int i = 0; i < monsterList.size(); i++)
        {
            ...
            if (monster.getType() == Monster.TYPE_BOMB)
            {
                        ...
                        , monster.getEndX(), monster.getStartY(), monster.getEndY()))
                {
                    ...
                    // 播放爆炸音效
                    GameManager.soundPool.play(
                            GameManager.soundMap.get(2), 1, 1, 0, 0, 1);
            ...
            for (Bullet bullet : bulletList)
            {
                ...
                if (monster.isHurt(bullet.getX(), bullet.getY()))
                {
                    ...
                    if(monster.getType() == Monster.TYPE_MAN)
                    {
                        // 播放惨叫音效
                        GameManager.soundPool.play(
                                GameManager.soundMap.get(3), 1, 1, 0, 0, 1);
                    }
