package com.simplegame.mymetalslug.ui;

import android.content.Context;
import android.graphics.Canvas;
import android.graphics.Paint;
import android.os.Handler;
import android.os.Looper;
import android.os.Message;
import android.view.MotionEvent;
import android.view.SurfaceHolder;
import android.view.SurfaceView;
import android.view.View;
import android.view.ViewGroup;
import android.widget.Button;
import android.widget.RelativeLayout;

import com.simplegame.mymetalslug.MainActivity;
import com.simplegame.mymetalslug.R;
import com.simplegame.mymetalslug.bean.Player;
import com.simplegame.mymetalslug.utils.MonsterManager;
import com.simplegame.mymetalslug.utils.GameManager;

import java.util.ArrayList;
import java.util.Collections;
import java.util.List;


public class GameView extends SurfaceView implements SurfaceHolder.Callback {

    public static final Player player = new Player("P1", Player.MAX_HP);
    private Context mContext;
    private Paint mPaint = null;
    private Canvas mCanvas = null;
    // 代表场景不改变的常量
    public static final int STAGE_NO_CHANGE = 0;
    // 代表初始化场景的常量
    public static final int STAGE_INIT = 1;
    // 代表登录场景的常量
    public static final int STAGE_LOGIN = 2;
    // 代表游戏场景的常量
    public static final int STAGE_GAME = 3;
    // 代表失败场景的常量
    public static final int STAGE_LOSE = 4;
    // 代表退出场景的常量
    public static final int STAGE_QUIT = 99;
    // 代表错误场景的常量
    public static final int STAGE_ERROR = 255;
    // 定义该游戏当前处于何种场景的变量
    private int currentStage = 0;
    // 定义一个集合来保存该游戏已经加载的所有场景
    public static final List<Integer> stageList =
            Collections.synchronizedList(new ArrayList<Integer>());
    private SurfaceHolder mSurfaceHolder;
    private GameThread mThread;
    private Handler setViewHandler = new Handler() {
        @Override
        public void handleMessage(Message msg) {
            super.handleMessage(msg);
            RelativeLayout layout = (RelativeLayout) msg.obj;
            if (layout != null) {
                RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(
                        ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
                MainActivity.mainLayout.addView(layout, params);
            }
        }
    };
    private Handler delViewHandler = new Handler() {
        @Override
        public void handleMessage(Message msg) {
            super.handleMessage(msg);
            RelativeLayout layout = (RelativeLayout) msg.obj;
            if (layout != null) {
                MainActivity.mainLayout.removeView(layout);
            }
        }
    };


    public GameView(Context context, int stage) {
        super(context);
        mContext = context;
        mPaint = new Paint();
        // 设置抗锯齿
        mPaint.setAntiAlias(true);
        // 设置该组件会保持屏幕常量，避免游戏过程中出现黑屏。
        setKeepScreenOn(true);
        // 设置焦点，相应事件处理
        setFocusable(true);
        mSurfaceHolder = getHolder();
        mSurfaceHolder.addCallback(this);
        //初始化屏幕大小
        GameManager.initScreen(MainActivity.windowWidth
                , MainActivity.windowHeight);
        currentStage = stage;
    }

    // 用于定义不同step的常量:
    private static final int INIT = 1; // 初始化
    private static final int LOGIC = 2; // 逻辑
    private static final int CLEAN = 3; // 清除
    private static final int PAINT = 4; // 绘图
    // 处理游戏场景
    public int doStage(int stage, int step) {
        int nextStage;
        switch (stage) {
            case STAGE_INIT:
                nextStage = doInit();
                break;
            case STAGE_LOGIN:
                nextStage = doLogin(step);
                break;
            case STAGE_GAME:
                nextStage = doGame(step);
                break;
            case STAGE_LOSE:
                nextStage = doLose(step);
                break;
            default:
                nextStage = STAGE_ERROR;
                break;
        }
        return nextStage;
    }
    
    // // TODO: 2017/3/4  
    private void stageLogic() {
        int newStage = doStage(currentStage, LOGIC);
        if (newStage != STAGE_NO_CHANGE && newStage != currentStage) {
            doStage(currentStage, CLEAN); // 清除旧的场景
            currentStage = newStage & 0xFF;
            doStage(currentStage, INIT);
        } else if (stageList.size() > 0) {
//            newStage = STAGE_NO_CHANGE;
            synchronized (stageList) {
                newStage = stageList.get(0);
                stageList.remove(0);
            }
            if (newStage == STAGE_NO_CHANGE) {
                return;
            }
            doStage(currentStage, CLEAN); // 清除旧场景
            currentStage = newStage & 0xFF;
            doStage(currentStage, INIT);
        }
    }

    // 执行初始化的方法
    private int doInit() {
        // 初始化游戏图片
        GameManager.loadResource();
        return STAGE_LOGIN;
    }

    // 定义登录界面
    private RelativeLayout loginLayout;
    private int doLogin(int step) {
        switch (step) {
            case INIT:
                player.setHp(Player.MAX_HP);
                if (loginLayout == null) {
                    loginLayout = new RelativeLayout(mContext);
                    loginLayout.setBackgroundResource(R.mipmap.game_back);
                    // 创建按钮
                    Button button = new Button(mContext);
                    button.setBackgroundResource(R.drawable.button_selector);
                    RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(
                            ViewGroup.LayoutParams.WRAP_CONTENT,
                            ViewGroup.LayoutParams.WRAP_CONTENT);
                    params.addRule(RelativeLayout.CENTER_IN_PARENT);
                    // 添加按钮
                    loginLayout.addView(button, params);
                    button.setOnClickListener(new OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            // 将游戏场景中的常量添加到stageList集合中
                            stageList.add(STAGE_GAME);
                        }
                    });
                    // 通过Handler通知主界面加载loginView组件
                    setViewHandler.sendMessage(setViewHandler.obtainMessage(0, loginLayout));
                }
                break;
            case LOGIC:
                break;
            case CLEAN:
                // 清楚登录界面
                if (loginLayout != null) {
                    delViewHandler.sendMessage(delViewHandler.obtainMessage(0, loginLayout));
                    loginLayout = null;
                }
                break;
            case PAINT:
                break;
        }
        return STAGE_NO_CHANGE;
    }

    // 定义游戏界面

    RelativeLayout gameLayout = null;
    private static final int ID_LEFT = 9000000;
    private static final int ID_FIRE = ID_LEFT + 1;

    private int doGame(int step) {
        switch (step) {
            case INIT:
                if (gameLayout == null) {
                    gameLayout = new RelativeLayout(mContext);
                    // 添加向左移动的按钮
                    Button button = new Button(mContext);
                    //noinspection ResourceType
                    button.setId(ID_LEFT);
                    button.setBackgroundResource(R.mipmap.left);
                    RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(
                            ViewGroup.LayoutParams.WRAP_CONTENT,
                            ViewGroup.LayoutParams.WRAP_CONTENT);
                    params.addRule(RelativeLayout.ALIGN_PARENT_LEFT);
                    params.addRule(RelativeLayout.ALIGN_PARENT_BOTTOM);
                    params.setMargins((int) (GameManager.scale * 20), 0, 0,
                            (int) (GameManager.scale * 10));
                    // 向游戏界面上添加向左的按钮
                    gameLayout.addView(button, params);
                    button.setOnTouchListener(new OnTouchListener() {
                        @Override
                        public boolean onTouch(View v, MotionEvent event) {
                            switch (event.getAction()) {
                                case MotionEvent.ACTION_DOWN:
                                    player.setMove(Player.MOVE_LEFT);
                                    break;
                                case MotionEvent.ACTION_UP:
                                    player.setMove(Player.MOVE_STAND);
                                    break;
                            }
                            return false;
                        }
                    });
                    // 添加向右移动的按钮
                    button = new Button(mContext);
                    button.setBackground(getResources().getDrawable(R.mipmap.right));
                    params = new RelativeLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
                            ViewGroup.LayoutParams.WRAP_CONTENT);
                    params.addRule(RelativeLayout.RIGHT_OF, ID_LEFT);
                    params.addRule(RelativeLayout.ALIGN_PARENT_BOTTOM);
                    params.setMargins((int) (GameManager.scale * 20), 0, 0,
                            (int) (GameManager.scale * 10));
                    gameLayout.addView(button, params);
                    button.setOnTouchListener(new OnTouchListener() {
                        @Override
                        public boolean onTouch(View v, MotionEvent event) {
                            switch (event.getAction()) {
                                case MotionEvent.ACTION_DOWN:
                                    player.setMove(Player.MOVE_RIGHT);
                                    break;
                                case MotionEvent.ACTION_UP:
                                    player.setMove(Player.MOVE_STAND);
                                    break;
                            }
                            return false;
                        }
                    });
                    // 添加射击按钮
                    button = new Button(mContext);
                    button.setId(ID_FIRE);
                    button.setBackgroundResource(R.mipmap.fire);
                    params = new RelativeLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
                            ViewGroup.LayoutParams.WRAP_CONTENT);
                    params.addRule(RelativeLayout.ALIGN_PARENT_RIGHT);
                    params.addRule(RelativeLayout.ALIGN_PARENT_BOTTOM);
                    params.setMargins(0, 0, (int) (GameManager.scale * 20),
                            (int) (GameManager.scale * 10));
                    gameLayout.addView(button, params);
                    button.setOnClickListener(new OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            // 当前角色的leftShootTime为0时(上一次射击结束), 才能发射下一发
                            if (player.getLeftShootTime() <= 0) {
                                player.addBullet();
                            }
                        }
                    });
                    // 添加跳跃按钮
                    button = new Button(mContext);
                    button.setBackgroundResource(R.mipmap.jump);
                    params = new RelativeLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
                            ViewGroup.LayoutParams.WRAP_CONTENT);
                    params.addRule(RelativeLayout.LEFT_OF, ID_FIRE);
                    params.addRule(RelativeLayout.ALIGN_PARENT_BOTTOM);
                    params.setMargins(0, 0, (int) (GameManager.scale * 20),
                            (int) (GameManager.scale * 10));
                    gameLayout.addView(button, params);
                    button.setOnClickListener(new OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            player.setJump(true);
                        }
                    });
                    setViewHandler.sendMessage(setViewHandler.obtainMessage(0, gameLayout));
                }
                break;
            case LOGIC:
                // 随机生存怪物
                MonsterManager.generateMonster();
                // 检查碰撞
                MonsterManager.checkMonster();
                // 角色跳跃与移动
                player.logic();
                if (player.isDead()) {
                    stageList.add(STAGE_LOSE);
                }
                break;
            case CLEAN:
                // 清楚游戏界面
                if (gameLayout != null) {
                    delViewHandler.sendMessage(delViewHandler.obtainMessage(0, gameLayout));
                    gameLayout = null;
                }
                break;
            case PAINT:
                // 绘制游戏元素
                GameManager.clearScreen(mCanvas);
                GameManager.drawGame(mCanvas);
                break;
        }
        return STAGE_NO_CHANGE;
    }

    // 游戏失败界面
    private RelativeLayout loseLayout;
    private int doLose(int step) {
        switch (step) {
            case INIT:
                // 初始化失败界面
                if (loseLayout == null) {
                    loseLayout = new RelativeLayout(mContext);
                    loseLayout.setBackgroundResource(R.mipmap.game_back);
                    Button button = new Button(mContext);
                    button.setBackgroundResource(R.mipmap.again);
                    RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(
                            ViewGroup.LayoutParams.WRAP_CONTENT,
                            ViewGroup.LayoutParams.WRAP_CONTENT);
                    params.addRule(RelativeLayout.CENTER_IN_PARENT);
                    loseLayout.addView(button, params);
                    button.setOnClickListener(new OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            // 跳转到继续游戏的界面
                            stageList.add(STAGE_GAME);
                            // 让角色的生命值回到最大值
                            player.setHp(Player.MAX_HP);
                        }
                    });
                    setViewHandler.sendMessage(setViewHandler.obtainMessage(0, loseLayout));
                }
                break;
            case CLEAN:
                if (loseLayout != null) {
                    delViewHandler.sendMessage(delViewHandler.obtainMessage(0, loseLayout));
                    loseLayout = null;
                }
                break;
        }
        return STAGE_NO_CHANGE;
    }

    @Override
    public void surfaceCreated(SurfaceHolder holder) {
        // 启动主线程执行部分
        mPaint.setTextSize(15);
        if (mThread != null) {
            mThread.needStop = true;
        }
        mThread = new GameThread(holder);
        mThread.start();
    }

    @Override
    public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {

    }

    @Override
    public void surfaceDestroyed(SurfaceHolder holder) {

    }

    // 两次绘制GameView之间默认的暂停时间
    public static final int SLEEP_TIME = 40;
    // 最小暂停时间
    public static final int MIN_SLEEP = 5;

    class GameThread extends Thread {

        public SurfaceHolder mSurfaceHolder = null;
        public boolean needStop = false;

        public GameThread(SurfaceHolder holder) {
            mSurfaceHolder = holder;
        }

        public void run() {
            long t1, t2;
            Looper.prepare();
            synchronized (mSurfaceHolder) {
                // 游戏未退出
                while (currentStage != STAGE_QUIT && !needStop) {
                    try {
                        // 处理游戏的场景逻辑
                        stageLogic();
                        t1 = System.currentTimeMillis();
                        mCanvas = mSurfaceHolder.lockCanvas();
                        if (mCanvas != null) {
                            // 处理游戏场景
                            doStage(currentStage, PAINT);
                        }
                        t2 = System.currentTimeMillis();
                        int paintTime = (int) (t2 - t1);
                        long millis = SLEEP_TIME - paintTime;
                        if (millis < MIN_SLEEP) {
                            millis = MIN_SLEEP;
                        }
                        // 该线程暂停millis毫秒后再次调用doStage()方法
                        sleep(millis);
                    } catch (InterruptedException e) {
                        e.printStackTrace();
                    } finally {
                        if (mCanvas != null) {
                            mSurfaceHolder.unlockCanvasAndPost(mCanvas);
                        }
                    }
                }
                Looper.loop();
                try {
                    sleep(1000);
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
            }
        }
    }
}
    

